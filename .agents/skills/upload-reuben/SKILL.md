---
name: upload-reuben
description: Add a Reuben review to Sergio's Sanity Studio using the Sanity CLI from rough notes, a grade, and a photo. Use when asked to upload a Reuben, add a Reuben review to Studio, or save/publish a Reuben on sergn.io. No browser involvement. Defaults to a draft; publishes only when explicitly requested.
---

# Upload Reuben

Turn rough notes and a photo into a complete `reubenReview` through the authenticated Sanity CLI. Do not open, automate, or depend on a browser. Do not build reusable scripts or new credential infrastructure for a content upload.
...
- Required of a recommendation: title, unique URL slug, restaurant, visit date, and a hero image with alt text.
- Required of a non-recommendation: title, unique URL slug, and the notes saying why it is not worth ordering again. Every other field stays optional, so fill in what is known and leave the rest unset rather than inventing a fuller record.
...
- Derive a concise title from the restaurant, adding a distinguishing detail only when two reviews of the same place would otherwise collide. Generate a lowercase, hyphen-separated slug of at most 96 characters; ask only if ambiguous. Check uniqueness across both drafts and published Reuben reviews.
- Ask a recommendation for the visit date if absent. The schema defaults `visitedAt` to today, which is a Studio convenience and not a fact: never let that default stand for a visit that happened on another day. Resolve relative dates against the user's local date and clarify ambiguous dates; use an approximate exact date only with user approval. Ask a non-recommendation once, accept that the date may not be remembered, and leave `visitedAt` unset rather than pressing for it.
...
- Optional: price, city, address, restaurant URL, notes, photo caption/credit, and publication date. Leave unknown optional fields unset. Store a supplied USD price as integer cents; clarify ambiguous currency.
- Inspect the image locally before writing factual alt text (5-180 characters). Describe visible subjects, not an imagined flavor or filename. A sandwich photographed cut in half is worth saying; it shows the fill.
- Strip EXIF before uploading a photo. Phone photos taken at a restaurant carry a GPS IFD, and the asset lands on a public CDN. `sips` preserves EXIF, so it is not sufficient on its own; drop the APP1/APP2 segments explicitly and verify they are gone.
- Convert unsupported images locally without changing the original. On macOS, for example: `sips -s format jpeg -Z 2000 /path/photo.HEIC --out /tmp/reuben-photo.jpg`. Verify the file type and inspect the converted image. Use a unique temporary directory to avoid collisions.
...
## Order details are what separate two Reubens

`orderDetails` is optional in the schema, but it is the part that makes one Reuben review worth reading next to another. Record what Sergio actually observed and leave the rest unset:

- `meat` is corned beef by default, but pastrami, turkey (a Rachel), or a vegetarian substitute are all common. Do not write "corned beef" unless it was corned beef.
- `bread` is usually marble or light rye, and whether it was griddled is worth capturing in the value.
- `cheese` is usually Swiss. `dressing` is usually Russian or Thousand Island, and the two are not the same; use whichever the menu or Sergio said.
- `sauerkraut` covers how it was handled: drained, wet, warm, house-made.
- `portion` is the size impression, such as "half sandwich" or "enormous, two-hands".
- `other` is an array of `{ label, value }` pairs for anything else, and both fields are required on each pair. Use it rather than stretching a field's meaning.

Never fill an order detail from what a Reuben usually is. An unstated ingredient stays unset.

## CLI workflow

### 1. Check for existing content

Query matching Reuben reviews by restaurant/title/date before uploading assets or creating documents. Use an authenticated query with API version `2021-06-07` for raw draft and published visibility:

```sh
npx sanity documents query '*[_type == "reubenReview" && (restaurant match "*Cecil*" || title match "*Cecil*")]' --api-version 2021-06-07 --project-id 0vbjaawm --dataset production
```
...
```sh
npx sanity assets upload --file /tmp/<unique-directory>/reuben.jpg --type image --filename reuben-<slug>.jpg --content-type image/jpeg --project-id 0vbjaawm --dataset production
```
...
- `visitedAt: "YYYY-MM-DD"` and `heroImage: { _type: "imageWithAlt", image: { _type: "image", asset: { _type: "reference", _ref: "<uploaded asset ID>" } }, alt: "..." }`. A recommendation owes the restaurant, the date, and the photo. On a non-recommendation each is optional: include the ones the user actually supplied and omit the rest entirely. Never fabricate a date, an ingredient, or a photo to fill the shape out.
- `orderDetails`: only the keys that are known. Each entry in `orderDetails.other` needs a unique `_key` plus both `label` and `value`.
...
An empty `[]` means no errors. Confirm the validator is actually resolving the schema rather than passing everything, for example by checking that an `other` pair missing its value is rejected. Inspect validation output, not just the exit status. Resolve errors from known facts, ask if information is missing, and check the content contract too. Never claim a complete draft or publish content with unresolved required-field errors.
...
**Recommended: the upload is not finished until the review has a rank.** The CLI cannot safely assign a unique `orderRank`: it is assigned by the Studio's drag-and-drop collection list. After saving and verifying the draft, stop and direct the user to open **Reuben reviews - Recommended** in the Studio and drag it into its intended position. Resume only once they confirm, then read the draft back and verify `orderRank` is present and unique among recommended Reuben reviews. If the draft already has an unchanged rank, still verify its uniqueness. An unranked or duplicate-ranked recommendation is an incomplete upload: do not publish it, and do not report it as done. Say plainly that it is waiting on a rank.
